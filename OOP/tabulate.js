...
function AbRows(A, b, xB, cB, ratio, pivRowIdx, pivColIdx, bools) {
    ...
    for (let i = 0; i < m; i++) {
        tempStr += "<tr>";
        tempStr += "<td>" + decimalToFrac(cB.matrix[i]) + "</td>";
        if (( pivRowIdx != i) || (isNaN(pivColIdx)) || isPermInf || isUnbounded 
        || isAltSol) {
            tempStr += subscripts(xB.matrix[i], {isBold: false, isLeftArrow: false, 
                isDownArrow: false, notRow: false});
        } else {
            tempStr += subscripts(xB.matrix[i], {isBold: true, isLeftArrow: true, 
                isDownArrow: false, notRow: false});
        }
        for (let j = 0; j < mn; j++) {
            tempStr += "<td>" + decimalToFrac(A.matrix[i][j]) + "</td>";
        }
        tempStr += "<td>" + decimalToFrac(b.matrix[i]) + "</td>";
        
        // Ratio column
        if (isFeas && !isOptim || befAltSol) {
            if (ratio.matrix[i] != Number.POSITIVE_INFINITY && ratio.matrix[i] >= 0) {
                tempStr += "<td>" + decimalToFrac(ratio.matrix[i]) + "</td>";
            } else {
                tempStr += "<td>NA</td>";
            }
        }
...
function genTableau(A, b, cj, x, xB, bools, pivotCol, ratio, pivotEl, 
    ...
    if (xB.matrix[pivRowIdx] == x.matrix[pivColIdx]) {
        pivRowIdx = NaN;
        pivColIdx = NaN;
    }
...
function headerRow(x, pivColIdx, bools) {
    ...
    for (let i = 0; i < x.height; i++) {
        if (i != pivColIdx || isPermInf || isAltSol) {
            tempStr += subscripts(x.matrix[i], {isBold: false, isLeftArrow: false, 
                isDownArrow: false, notRow: false});
        } else {
            tempStr += subscripts(x.matrix[i], {isBold: true, isLeftArrow: false, 
...
function objectiveRow(cj) {
    ...
    for (let i = 0; i < cj.height; i++) {
        tempStr += "<td>" + decimalToFrac(cj.matrix[i]) + "</td>";
    }
    tempStr += "</tr>";
}
...
function ratRow(pivotEl, ratio, bools) {
    ...
    if (ratio != undefined && !isNaN(pivotEl) && !isFeas && !isPermInf) {
        ...
        tempStr += "<td>" + katex.renderToString("\\mathrm{Ratio}") + "</td>";
        for (let i = 0; i < mn; i++) {
            if (ratio.matrix[i] != Number.POSITIVE_INFINITY) {
                tempStr += "<td>" + decimalToFrac(ratio.matrix[i]) + "</td>";
            } else {
                tempStr += "<td></td>";
            }
...
function rowOperations(pivRowIdx, pivotCol, pivotEl) {
    ...
    for (let i = 0; i < m; i++) {
        ...
        // of the pivot column element for the row
            ...
            if (pivotCol.matrix[i] == -1) {
                tempStr += "<div style='padding: 7px;'>" + 
                 katex.renderToString("R_{" + (i + 1) + "} + " + "R_{" + 
                 pivRowIdx + "}^{'} \\rightarrow R_{" + (i + 1) + "}^{'}") + 
                 "</div>";
            } else if ( pivotCol.matrix[i] < 0) {
                var fraction = math.fraction(-pivotCol.matrix[i]);
                if (fraction.d != 1) {
                    tempStr += "<div style='padding: 7px;'>" + 
...
function zcRow(zc) {
    ...
    for (let i = 0; i < mn; i++) {
        tempStr += "<td>" + decimalToFrac(zc.matrix[i]) + "</td>";
    }
    tempStr += "</tr>";
}
...
function zRow(pivotEl, isFeas, ratio, z) {
    ...
    for (let i = 0; i < mn; i++) {
        tempStr += "<td>" + decimalToFrac(z.matrix[i]) + "</td>";
    }

    // Objective function value
    tempStr += "<td rowspan='2'>" + decimalToFrac(z.matrix[mn]) + "</td>";
    tempStr += "</tr>";
}